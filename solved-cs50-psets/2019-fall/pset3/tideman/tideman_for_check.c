...
void add_pairs(void);
void sort_pairs();
void sorting_merge(int min, int max);
...
void sort_pairs()
{
    sorting_merge(0, (pair_count * (pair_count - 1) / 2) - 1);
}
...
void sorting_merge(int min, int max)
{
    ...
    if ((max + min) % 2 == 0)
    {
        ...
        sorting_merge(min, middle - 1);

        sorting_merge(middle, max);
    }
    // Fictive splitting an array in half if the sum is odd
    else
    {
        // Sorting left and right halves
        sorting_merge(min, middle);

        sorting_merge(middle + 1, max);
    }
