...
def add():
    ...
    # User reached route via POST (as by submitting a form via POST)
        ...
        # Ensure a user provided a positive number and less than or equal to 1.000.000,00
        elif float(cash) < 0.01 or float(cash) > 1000000:
            return apology("Only positive integers are allowed and less than or equal to 1.000.000,00")

        # Converting cash to float
        cash = float(cash)
        # Detirmine how much cash a user has
        user_cash = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=user_id)[0]["cash"]

        # Ensure that user won't have more than 9.999.000 USD
        if user_cash + cash > 9999000.00:
            return apology("You cannot have more than 9.999.000,00 USD on your account")
...
def withdraw():
    ...
    # User reached route via POST (as by submitting a form via POST)
        ...
        # Ensure a user provided a positive number
        elif float(cash) <= 0:
